...
export class PuckHouse extends Area {
  ...
  buildSideboard() {
    ...
    // Net als in de video: bovenop de rugleuning van de bank, met een blikje cola ernaast
    bag.position.set(-4.81, 0.9, 1.6);
    bag.rotation.y = Math.PI / 2 + 0.08;
    bag.scale.set(1, 1, 0.9);
    bag.traverse((m) => (m.castShadow = true));
    this.group.add(bag);
    this.zones.push({ x: -4.6, y: 0, z: 1.6, r: 0.75, h: 2, secret: 'tasje', dance: true });
    const can = new THREE.Group();
    const canBody = new THREE.Mesh(new THREE.CylinderGeometry(0.033, 0.033, 0.12, 14), lambert(0x14224a));
    const band = new THREE.Mesh(new THREE.CylinderGeometry(0.0335, 0.0335, 0.03, 14), lambert(0xd7263d));
    band.position.y = 0.012;
    const band2 = new THREE.Mesh(new THREE.CylinderGeometry(0.0336, 0.0336, 0.012, 14), M.white);
    band2.position.y = -0.008;
    const lid = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.033, 0.008, 14), M.metal);
    lid.position.y = 0.064;
    can.add(canBody, band, band2, lid);
    can.position.set(-4.81, 0.91, 2.75);
    can.traverse((m) => (m.castShadow = true));
    this.group.add(can);
    this.zones.push({ x: -4.81, y: 0.85, z: 2.75, r: 0.3, h: 0.5, secret: 'cola', sound: 'puck-watskecola', say: 'Watskecola!' });
